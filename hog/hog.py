...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        ...
        total += m
        num_rolls -= 1
    return total if not has_one else 1
...
def more_boar(player_score, opponent_score):
    ...
    while True:
        fc = player_score % 10  # 6
        sc = opponent_score % 10  # 8
        if fc < sc:
            step += 1  # 1
        player_score //= 10  # 0
        opponent_score //= 10  # 1
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 6
    player = 0  # 题目已知 player0 先手
    step = 0
    while True:
        if score0 >= goal or score1 >= goal:
            break
        if player == 0:  # 6 + 6+ 6 + 6 + 6
            ...
            if not more_boar(score1, score0):
                player = next_player(player)
        say = say(score0, score1)  # 定义的返回函数，意思就是有个return 条件
